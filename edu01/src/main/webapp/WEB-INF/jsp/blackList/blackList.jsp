...
<%@ page import = "edu.seedit.vo.BlackListVO" %>
...
<div>
	<jsp:include page="/WEB-INF/jsp/inc/menu_header.jsp">
			<jsp:param name="curMenu" value="blackList" />
	</jsp:include>
</div>
...
List<BlackListVO> resultList = (List<BlackListVO>) request.getAttribute("resultList"); 
...
<table>
	...
	<tbody>
		<form action="<%=contextPath %>/blackList/blackList.do" method="post" name="searchForm">
			ID조회 : <input type="text" class="wd100" id="tags"
				name="listID" value="<%=listID %>" style="width:30%;display:inline">	 
				<input type="submit" class="btn_common_red_2" value="검색">
				<input type="hidden" name="pageNo" value="${paging.pageNo}"}> 
		</form><%-- 
		<tr>
			<th>listID         :</th><th class="wd110"><input class="wd100" id="tags" name="listID"         value="${listID}"/></th>
			<th>
				<input type="submit" class="btn_common_red_2" value="검색">
				<input type="hidden" name="pageNo" value="${paging.pageNo}">
			</th>
...
<div>
	...
	<a href="<%=contextPath%>/" class="btn_common_red_2">글쓰기</a> 
	<a href="<%=contextPath%>/sample/openSampleList.do" class="btn_common_red_2">사원리스트보기</a> <!-- 
	...
	<input type="button" id="seeBlackList" class="btn_common_red_2" value="사원스트보기"> -->
</div>
...
<table class="table_list">
		...
		<c:choose>
			<c:when test="${fn:length(list) > 0}">
				<c:forEach items="${list }" var="row">
					<tr>
						<td class="text_ct"><input type="checkbox" name="checkRow" value="${content.listID}" /></td>
						<td><a href="<%=contextPath %>/sample/detail.do?listID=${row.listID}"> ${row.listID}</a></td>
						<td>${row.listID      }</td>
						<td>${row.sender     }</td>
						<td>${row.recName     }</td>
						<td>${row.received      }</td>
	...
	function fn_openSampleWrite(){
		location.href="<c:url value='/blackList/blackListWrite.do' />"
	}
